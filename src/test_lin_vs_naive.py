...
def test_random():
    ...
    assert(sorted(naive_runner(fasta100,fastq4))==sorted(lin_runner(fasta100,fastq4)))

    fastq100 = fastq_func(random_sequence_generator(100, 8, 'fastq','fastq100'))

    assert(sorted(naive_runner(fasta100,fastq100))==sorted(lin_runner(fasta100,fastq100)))
...
def test_genome():
    ...
    assert(sorted(naive_runner(gen,fastq0))==sorted(lin_runner(gen,fastq0)))
    assert(sorted(naive_runner(gen,fastq1))==sorted(lin_runner(gen,fastq1)))
    assert(sorted(naive_runner(gen,fastq10))==sorted(lin_runner(gen,fastq10)))
    assert(sorted(naive_runner(gen,fastq25))==sorted(lin_runner(gen,fastq25)))
